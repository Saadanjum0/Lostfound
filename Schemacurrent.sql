...
CREATE TABLE public.conversation_participants (
  ...
  role character varying DEFAULT 'member'::character varying CHECK (role::text = ANY (ARRAY['admin'::character varying::text, 'member'::character varying::text])),
...
CREATE TABLE public.conversations (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  type character varying DEFAULT 'direct'::character varying CHECK (type::text = ANY (ARRAY['direct'::character varying::text, 'group'::character varying::text])),
...
CREATE TABLE public.message_status (
  ...
  message_id uuid NOT NULL,
  user_id uuid NOT NULL,
  status character varying DEFAULT 'sent'::character varying CHECK (status::text = ANY (ARRAY['sent'::character varying::text, 'delivered'::character varying::text, 'read'::character varying::text])),
...
CREATE TABLE public.user_relationships (
  ...
  relationship_type character varying NOT NULL CHECK (relationship_type::text = ANY (ARRAY['friend'::character varying::text, 'blocked'::character varying::text, 'following'::character varying::text])),
  status character varying DEFAULT 'active'::character varying CHECK (status::text = ANY (ARRAY['active'::character varying::text, 'pending'::character varying::text, 'declined'::character varying::text])),
  created_at timestamp with time zone DEFAULT now(),
